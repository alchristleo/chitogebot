...
class MusicBot(discord.Client):
    ...
    async def update_now_playing(self, entry=None, is_paused=False):
        ...
        await self.change_status(game=discord.Game(name="with Alchrist | -help", type=1))
    ...
    async def cmd_id(self, author, user_mentions):
        ...
        """
        if not user_mentions:
            return Response('your id is `%s`' % author.id, reply=False, delete_after=35)
        else:
            usr = user_mentions[0]
            return Response("%s's id is `%s`" % (usr.name, usr.id), reply=False, delete_after=35)

    async def cmd_chitoge(self, message):
        return Response("https://goo.gl/Dzj2Ui", delete_after=60)

    async def cmd_ascii(self, message):
        return Response("https://goo.gl/D5Kdg7", delete_after=60)

    async def cmd_quotes(self, message):
        quotes = ['“You cant achieve anything if you dont make a move. Love is an all-out assault. - Marika Tachibana',
        '“There is no law that says you cant go after someone just because he is already in love with someone else. - Ruri Miyamoto',
        '“In the end, mustering the courage to confess is up to the people concerned. - Shuu Maiko', 'From here on, Im going to make sure you dont forget me! - Marika Tachibana', 'Dont worry when I fight with you... Worry when I stop because there is nothing left for us to fight for. - Chitoge Kirisaki', 'Never lose yourself when trying to hold on to someone who doesnt care about losing you. - Chitoge Kirisaki', 'Sometimes knowing something... Is worse than not knowing anything - Chitoge Kirisaki', 'I really hate it when I miss you so much and I cant do anything about it. - Kosaki Onodera']
        secure_random = random.SystemRandom()
        return Response(secure_random.choice(quotes), reply=False, delete_after=60)

    async def cmd_ratewaifu(self, message, anime_name):
        if anime_name == "chitoge":
            return Response("I rate waifu Chitoge a 100/100", delete_after=60)
        else:
            return Response('I rate waifu %s a %d/100' % (anime_name, random.randint(0,100)), reply=False, delete_after=60)

    async def cmd_givechitoge(self, message, item_name):
        thankyoumsg = "Hi %s thank you for giving me %s (　＾∇＾)\n\n" % (message.author.mention, item_name)
        thankyoumsg += "http://pa1.narvii.com/5812/e58626708b656d35b63677e6f507d39a29bc607e_hq.gif"
        return Response(thankyoumsg, reply=False, delete_after=60)

    async def cmd_about(self, message):
        aboutmsg = "Hi %s, My name is **Chitoge-Bot**. My master is **Alchrist Leo**. Hope you enjoy my service and have a nide day (ﾉ^∇^)ﾉﾟ\n" % (message.author.mention)
        aboutmsg += "http://24.media.tumblr.com/f247d4f4d442c412d0fb7cfa4eb60129/tumblr_mzmejhAy8L1shiho3o1_500.gif"
        return Response(aboutmsg, reply=False, delete_after=60)

    async def cmd_fact(self, message):
        factmsg = "**Age:** 17\n"
        factmsg += "**Birthday:** June 7\n"
        factmsg += "**Zodiac sign:** Gemini\n"
        factmsg += "**Height:** 160 cm\n"
        factmsg += "**Blood type:** B\n"
        factmsg += "**Likes:** Raku Ichijou, ramen, her ribbon\n"
        factmsg += "**Dislikes:** Claude's over-protectiveness, being called a gorilla by Raku, Dorayaki\n"
        factmsg += "**Abilities:** Immense Strength, Great Agility\n"
        factmsg += "**Weakness:** awful cooking, lightning, darkness\n\n"
        factmsg += "https://s-media-cache-ak0.pinimg.com/originals/58/0c/af/580cafaa18b8da3d17de1ed1be3ff0fd.jpg"
        return Response(factmsg, reply=False, delete_after=60)

    async def cmd_create_ascii(self, message, user_mentions):
        if not user_mentions:
            return Response("You must mention a user", reply=False, delete_after=60)
        else:
            usr = user_mentions[0]
            msg = "http://artii.herokuapp.com/make?text=%s" % (usr.name)
            return Response(msg, reply=False, delete_after=60)
    ...
    async def cmd_marry(self, message, user_mentions):
        if not user_mentions:
            ...
            usr = user_mentions[0]
            marrymsg = "Congratulations! Happy wedding %s and **%s**\n\n" % (message.author.mention, usr.name)
            marrymsg += "http://pa1.narvii.com/5812/39e48a13e4c123614973cd6b0fff6a41afd9df27_hq.gif"
            return Response(marrymsg, reply=False, delete_after=60)

    async def cmd_kiss(self, message, user_mentions):
        if not user_mentions:
            return Response("You must mention a user", reply=False, delete_after=60)
        else:
            kissmsg = "So sweet (*´∀｀*人*´∀｀*) (*´∀｀*人*´∀｀*) (*´∀｀*人*´∀｀*)\n\n"
            kissmsg += "http://pm1.narvii.com/5812/8cc3af185b4c8c6b33e36c04168350705e0e1d25_hq.jpg"
            return Response(kissmsg, reply=False, delete_after=60)
    ...
    async def cmd_hug(self, message, user_mentions):
        if not user_mentions:
            ...
            usr = user_mentions[0]
            hugmsg = "**%s**, you've been hugged by %s (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ\n\n" % (usr.name, message.author.mention)
            hugmsg += "https://media.tenor.com/images/35f8ecc9f2262bc25869e6c32793d0d7/tenor.gif"
            return Response(hugmsg, reply=False, delete_after=60)

    async def cmd_coinflip(self, message):
        coinflip = ['Head', 'Tail']
        secure_random = random.SystemRandom()
        return Response("You've flipped **%s**" % (secure_random.choice(coinflip)), reply=False, delete_after=60)

    async def cmd_roll(self, message):
        return Response("You've rolled **%d**" % (random.randint(0,10000)), reply=False, delete_after=60)

    async def cmd_randomgif(self, message):
        randomgif = ['https://media.tenor.com/images/93c79c0ec87f8227f9112820764e2fa7/tenor.gif', 'https://media.tenor.com/images/adad0de5a9b93a2e6006eb4971c74dd9/tenor.gif', 'https://media.tenor.com/images/7fd7f3df2aec99448ea7cdcee539bf79/tenor.gif', 'https://media.tenor.com/images/492d8b23890678a9fe0d6426191ef536/tenor.gif', 'https://media.tenor.com/images/637ce537067821039b2c414f9d070518/tenor.gif', 'https://media.tenor.com/images/da7b66634fd416ff60b386ef3cb7d32b/tenor.gif', 'https://media.tenor.com/images/19ff43db8e36c26fb80e0ac50ae7f55f/tenor.gif']
        secure_random = random.SystemRandom()
        return Response(secure_random.choice(randomgif), reply=False, delete_after=60)

    async def cmd_gachigasm(self, message):
        gachigasmmsg = "░░░░░▓▓▓▓▓▓▓▓▓▓▓░░░░░░░░\n"
        gachigasmmsg += "░░░▓▓▓▓▓▓▒▒▒▒▒▒▓▓░░░░░░░\n"
        gachigasmmsg += "░░▓▓▓▓▒░░▒▒▓▓▒▒▓▓▓▓░░░░░\n"
        gachigasmmsg += "░▓▓▓▓▒░░▓▓▓▒▄▓░▒▄▄▄▓░░░░\n"
        ...
        gachigasmmsg += "░░░░░▓▓▒░░▒▒▒▒▒▒▒▒▒▒▒▓▓░\n"
        gachigasmmsg += "░░░░░░▓▒▒░░░░▒▒▒▒▒▒▒▓▓░░"
        return Response(gachigasmmsg, reply=False, delete_after=60)

    async def cmd_kappa(self, message):
        kappamsg = "░░░░░░░░░\n"
        kappamsg += "░░░░▄▀▀▀▀▀█▀▄▄▄▄░░░░\n"
        kappamsg += "░░▄▀▒▓▒▓▓▒▓▒▒▓▒▓▀▄░░\n"
        kappamsg += "▄▀▒▒▓▒▓▒▒▓▒▓▒▓▓▒▒▓█░\n"
        ...
        kappamsg += "░░░░░▀█▄▒▒░░░░▒▄▀░░░\n"
        kappamsg += "░░░░░░░░▀▀█▄▄▄▄▀\n"
        return Response(kappamsg, reply=False, delete_after=60)

    async def cmd_trihard(self, message):
        trihardmsg = "░░░░░░░▄░░░░░░░░░░░░ \n"
        trihardmsg += "░░░░▄▄░██▓██▓▓▄░░░░░ \n"
        trihardmsg += "░░░░▓███▓▓██▓██▄▄░░░ \n"
        trihardmsg += "░░▄█▓██▓▓██▓██▓▓█▄░░ \n"
        ...
        trihardmsg += "░░░░░░▓░▀▄▄▌▓░░░░░░░ \n"
        trihardmsg += "░░░░░░░▓▓▓▓▓░░░░░░░░\n"
        return Response(trihardmsg, reply=False, delete_after=60)

    async def cmd_pogchamp(self, message):
        pogchampmsg = "░░░░░▄██████████▄▄░░░\n"
        pogchampmsg += "░░░░▄██████████████▐░░\n"
        pogchampmsg += " ░░▄█████████▀▀▓▀███░░░\n"
        pogchampmsg += " ░▄███████▓▒▒▒▒▒▒▓▓█▐░░\n"
        ...
        pogchampmsg += " ░░░░░░▀▄▓▒▒▒▒▒▀▀░░░░░░\n"
        pogchampmsg += "░░░░░░░░▀▄▄▒▒▓▄▐░░░░░░\n"
        return Response(pogchampmsg, reply=False, delete_after=60)

    async def cmd_my_mmr(self, message, author):
        if author.id == self.config.owner_id:
            my_mmr = "**Это ГГ** %s, your MMR is **%d**" % (message.author.mention, random.randint(9000,10000))
        else:
            my_mmr = "**Ай-ай-ай-ай-ай, что сейчас произошло!** %s, your MMR is **%d**" % (message.author.mention, random.randint(0,5000))
        return Response(my_mmr, reply=False, delete_after=60)

    async def cmd_my_dong(self, message):
        x = random.randint(5,25)
        if x <= 15:
            my_dong = "**TOO SMALL** :persevere: :weary: :cry: %s, your dong length is **%d cm**\n\n" % (message.author.mention, x)
        else:
            my_dong = "**SUGOIII, KIMOCHI NA (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ** %s, your dong length is **%d cm**\n\n" % (message.author.mention, x)
        return Response(my_dong, reply=False, delete_after=60)

    async def cmd_my_iq(self, message, author):
        if author.id == self.config.owner_id:
            my_iq = "**SUGOIII (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ** %s, your IQ is **%d**" % (message.author.mention, random.randint(150, 200))
        else:
            my_iq = "**BAKA!! (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ** %s, your IQ is **%d**" % (message.author.mention, random.randint(70, 120))
        return Response(my_iq, reply=False, delete_after=60)

    async def cmd_my_ip(self, message, author):
        if author.id == self.config.owner_id:
            my_ip = "**SUGOIII (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ** %s, your IP is **%.2f**" % (message.author.mention, random.random()*(0.50)+3.50)
        else:
            my_ip = "**BAKA!! (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ** %s, your IP is **%.2f**" % (message.author.mention, random.random()*(1.00)+2.50)
        return Response(my_ip, reply=False, delete_after=60)

    async def cmd_katomegumi(self, message):
        katomegumi = "https://amagavn.com/wp-content/uploads/2017/03/Megumi-Kato.gif"
        return Response(katomegumi, reply=False, delete_after=60)

    async def cmd_istanaboneka(self, message):
        songlyric = "♫♫ Lihat indahnya dunia ♫♫\n"
        songlyric += "♫♫ Milik kita semua ♫♫\n"
        songlyric += "♫♫ Walau berbeda bangsa ♫♫\n"
        songlyric += "♫♫ Namun satu sodara ♫♫\n"
        songlyric += "♫♫ Alangkah indah﻿ semua ♫♫\n"
        songlyric += "♫♫ Lautan luas ♫♫\n"
        songlyric += "♫♫ Angkasa luas ♫♫\n"
        songlyric += "♫♫ Gemah riang bumi pertiwi ♫♫\n"
        songlyric += "♫♫ Kamu harapan suci ♫♫\n"
        songlyric += "♫♫ Bahagia gembira semua ♫♫\n"
        songlyric += "♫♫ Satu tujuan kita ♫♫\n"
        songlyric += "♫♫ Milik kita semuanya ♫♫\n"
        songlyric += "♫♫ Hidup senang bersama ♫♫\n"
        songlyric += "♫♫ Damai bebas merdeka ♫♫\n"
        return Response(songlyric, reply=False, delete_after=60)

    async def cmd_kizunaai(self, message):
        kizunaaimsg = "https://9gag.com/gag/aB8Wx8D"
        return Response(kizunaaimsg, reply=False, delete_after=60)
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        if voice_client and voice_client.channel.server == author.voice_channel.server:
            welcomemsg = "**OHAYOU SENPAI (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ\n\n**"
            welcomemsg += "https://goo.gl/d961iU"
            await self.move_voice_client(author.voice_channel)
            return Response(welcomemsg, reply=False, delete_after=60)
    ...
    async def cmd_clear(self, player, author, channel, permissions):
        """
        Usage:
        ...
        Clears the playlist.
        """
        clearmsg = "**Song Queue List has been Cleared (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ\n\n**"
        clearmsg += "http://imgur.com/WXYqSzB"

        player.playlist.clear()
        return Response(clearmsg, delete_after=20)
    ...
    async def cmd_volume(self, message, author, user_mentions, player, new_volume=None):
        """
        Usage:
        ...
        if not new_volume:
            return Response('Current volume: `%s%%`' % int(player.volume * 100), reply=False, delete_after=20)
        ...
        if 0 < new_volume <= 100:
            ...
            volumemsg = "**OHAYOU SENPAI %s (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ** Volume updated from %d to %d\n" % (message.author.mention,old_volume, new_volume)
            volumemsg += "\nhttp://imgur.com/WXYqSzB"
            return Response(volumemsg, reply=False, delete_after=45)
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        cleanmsg = "Cleaned up {} message{}.".format(deleted, 's' * bool(deleted))
        cleanmsg += "\nhttp://imgur.com/WXYqSzB"
        return Response(cleanmsg, delete_after=60)
    ...
    async def cmd_disconnect(self, server):
        disconnectmsg = "**GOODBYE SENPAI (づ｡◕‿‿◕｡)づ(づ｡◕‿‿◕｡)づ\n\n**"
        disconnectmsg += "https://goo.gl/d961iU"
        await self.disconnect_voice_client(server)
        return Response(disconnectmsg, delete_after=60)
    ...
    async def on_message(self, message):
        ...
        try:
            ...
            if response and isinstance(response, Response):
                ...
                if response.reply:
                    content = 'こんにちは %s、私の名前は **Chitoge-Bot** です。 私のマスターは **Alchrist Leo** です。 私を使用していただきありがとうございます、あなたが楽しむことを楽しみにして、良い一日を (ﾉ^∇^)ﾉﾟ, %s' % (message.author.mention, content)
